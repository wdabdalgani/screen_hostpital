(function () {
  ...
  /** قائمة محتوى جديدة تنتظر نهاية الدورة الحالية قبل تطبيقها */
  var pendingContents = null;
  var pendingContentPlaylistSig = '';
  ...
  var DEPT_PATHS = {
    ...
    hospital:
      '<path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M17 21v-8a1 1 0 0 0-1-1h-1"/><path d="M7 21v-8a1 1 0 0 1 1-1h1"/><path d="M17 21h2v-4h-2"/><path d="M5 21H3v-4h2"/><path d="M5 21V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16"/>',
  ...
  function playlistSignature(items, key) {
    return (items || [])
      .map(function (it) {
  ...
  function swap() {
    ...
    // في وضع المحتوى: لا نقطع الدورة الحالية؛ نطبق قائمة جديدة فقط بعد اكتمالها.
    if (displayMode === 'content' && nextIndex === 0 && pendingContents && pendingContents.length) {
      contents = pendingContents;
      lastContentPlaylistSig = pendingContentPlaylistSig;
      pendingContents = null;
      pendingContentPlaylistSig = '';
      arr = contents;
      nextIndex = 0;
    }
    index = nextIndex;
  ...
  function applyData(data) {
    ...
    var nextDoctors = data.doctors || [];
    var nextContents = data.contents || [];
    ...
    if (displayMode === 'content') {
      ...
      var contentSig = playlistSignature(nextContents, 'id');
      if (!contents.length) {
        contents = nextContents;
      ...
      if (contentSig !== '' && contentSig === lastContentPlaylistSig) {
        // نفس القائمة: حدّث بيانات العناصر (مثل المدة) دون إعادة البدء من أول شريحة.
        contents = nextContents;
        pendingContents = null;
        pendingContentPlaylistSig = '';
        lastContentPlaylistSig = contentSig;
      ...
      // قائمة مختلفة: انتظر اكتمال الدورة الحالية ثم بدّل للقائمة الجديدة.
      pendingContents = nextContents;
      pendingContentPlaylistSig = contentSig;
      scheduleSlide();
      return;
    }
    ...
    var doctorSig = playlistSignature(doctors, 'id');
  ...
  /* أول لمسة/نقرة: طلب ملء الشاشة (إخفاء شريط المتصفح حيث يُسمح) + قفل اتجاه عمودي إن وُجد */
