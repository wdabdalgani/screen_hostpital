...
function run_migrations(PDO $pdo): void
{
    ...
    if (!in_array(17, $done, true)) {
        migrate_v17($pdo);
        try {
            $pdo->prepare('INSERT INTO schema_migrations (version) VALUES (17)')->execute();
        } catch (Throwable $e) {
            // duplicate
        }
    }

    if (!in_array(18, $done, true)) {
        migrate_v18($pdo);
        try {
            $pdo->prepare('INSERT INTO schema_migrations (version) VALUES (18)')->execute();
        } catch (Throwable $e) {
            // duplicate
        }
    }

    if (!in_array(19, $done, true)) {
        migrate_v19($pdo);
        try {
            $pdo->prepare('INSERT INTO schema_migrations (version) VALUES (19)')->execute();
        } catch (Throwable $e) {
            // duplicate
        }
    }
...
function migrate_v17(PDO $pdo): void
{
    // Previously registered wide_19_6 preset; removed project-wide — see migrate_v18.
}

function migrate_v18(PDO $pdo): void
{
    if (table_exists($pdo, 'screens')) {
        try {
            $pdo->exec("UPDATE screens SET display_style = 'hero_medical' WHERE display_style = 'wide_19_6'");
        } catch (Throwable $e) {
            // ignore
        }
    }
    ...
    try {
        $pdo->exec("DELETE FROM display_styles WHERE style_key = 'wide_19_6'");
    } catch (Throwable $e) {
        // ignore
    }
}

function migrate_v19(PDO $pdo): void
{
    if (table_exists($pdo, 'hospital_settings') && !column_exists($pdo, 'hospital_settings', 'default_display_shell')) {
        try {
            $pdo->exec(
                "ALTER TABLE hospital_settings ADD COLUMN default_display_shell VARCHAR(16) NOT NULL DEFAULT 'classic' COMMENT 'classic|hariri' AFTER social_youtube"
            );
        } catch (Throwable $e) {
            // ignore
        }
